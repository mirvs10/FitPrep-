...
function Meals() {
  ...
  return (
    <MockupShell breadcrumbs={["FitKitchen", "Catálogo"]}>
      <div className="p-8">
        ...
        <Card className="overflow-hidden">
          <div className="p-4 border-b border-border flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 px-3 h-9 rounded-md bg-muted text-sm flex-1 min-w-[200px] max-w-md">
              ...
              <input className="bg-transparent flex-1 outline-none" placeholder="Buscar comida..." />
            </div>
            {["Todas","Activas","Borrador","Inactivas"].map((t,i) => (
              <button key={t} className={`px-3 h-8 rounded-md text-xs font-medium ${i===0?"bg-foreground text-background":"border border-border hover:bg-muted"}`}>{t}</button>
            ))}
          </div>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="px-5 py-8 text-center text-muted-foreground text-sm">No tienes platos registrados. Crea uno nuevo.</td></tr>
              ) : (
                platos?.map((plato) => (
                  <tr key={plato.id} className="hover:bg-muted/40">
                    <td className="px-5 py-3"><div className="size-9 rounded-lg bg-gradient-to-br from-brand-100 to-brand-50" /></td>
